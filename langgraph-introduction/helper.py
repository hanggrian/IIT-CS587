...
class ewriter():
    ...
    def research_plan_node(self, state: AgentState):
        queries = \
            self.model.with_structured_output(Queries).invoke([
...
class writer_gui():
    ...
    def get_state(self, key):
        ...
        if key in current_values.values:
            ...
            new_label = \
                f'last_node: {lnode}, thread_id: {self.thread_id}, rev: {rev}, step: {astep}'
    ...
    def create_interface(self):
        with gr.Blocks(theme=gr.themes.Default(spacing_size='sm', text_size='sm')) as demo:
                    ...
                    return {
                        ...
                        step_pd: \
                            gr.Dropdown(
                                label= \
                                    'update_state from: ' + \
                                    'thread:count:last_node:next_node:rev:thread_ts',
                                choices=hist,
                                value=hist[0],
                                interactive=True,
                            ),
            ...
            with gr.Tab('Agent'):
                ...
                thread_pd \
                    .input(self.switch_thread, [thread_pd], None) \
                    .then(fn=updt_disp, inputs=None, outputs=sdisps)
                step_pd \
                    .input(self.copy_state, [step_pd], None) \
                    .then(fn=updt_disp, inputs=None, outputs=sdisps)
                gen_btn \
                    .click(vary_btn, gr.Number('secondary', visible=False), gen_btn) \
                    .then(
                    fn=self.run_agent,
                    ...
                    .then(fn=updt_disp, inputs=None, outputs=sdisps) \
                    .then(vary_btn, gr.Number('primary', visible=False), gen_btn) \
                    .then(vary_btn, gr.Number('primary', visible=False), cont_btn)
                cont_btn \
                    .click(vary_btn, gr.Number('secondary', visible=False), cont_btn) \
                    .then(
                    fn=self.run_agent,
